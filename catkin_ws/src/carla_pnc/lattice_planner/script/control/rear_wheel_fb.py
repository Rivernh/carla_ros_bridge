...
class CapacController(object):
    def __init__(self, world, vehicle, frequency):
        ...
        self.acc = 0
        self.steer = 0
        self.PIDer = IncreasPID(kp=2.00,ki=0.005,kd=0.5)
    ...
    def run_step_ros(self, trajectory, index, state0,currentstate):
        '''
        Args:
        ...
        current_state.time_stamp =time_stamp
        current_state = current_state.world_to_local_2D(state0, 'base_link')

        x, y = trajectory['x'][index], trajectory['y'][index]
        a, v = trajectory['a'][index], trajectory['v'][index]
        theta = trajectory['yaw'][index]
        t = trajectory['time']
        k = trajectory['c'][index]
        # print(f"target v:{v}")
        ...
        # steer here is minus of cuclation!!!!!!
        throttle, brake = self.pid_new(current_state, target_state)
        ...
        if throttle > 0 and abs(current_state.v) < 0.05 and abs(target_state.v) < 0.05:
            throttle = 0.
            brake = 1.
            steer = 0.
        ks = 0.3
    ...
    def pid_new(self, current_state, target_state):
        v_current = current_state.v
        v_target = target_state.v

        self.acc += self.PIDer.run(v_target,v_current)

        throttle = np.clip(self.acc, 0, self.max_a)
        brake = -np.clip(self.acc, self.min_a, 0)
