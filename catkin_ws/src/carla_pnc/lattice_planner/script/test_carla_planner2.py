...
class obstacle_vehicle(object):
    def __init__(self,vehicle):
        '''vehicle'''
        self.vehicle = vehicle
        tansform = vehicle.get_transform()
        vel = vehicle.get_velocity()
        self.pose = [tansform.location.x,tansform.location.y,tansform.rotation.yaw]
        self.vel = [vel.x,vel.y]
        self.speed = math.sqrt(vel.x**2 + vel.y**2)
...
def get_plan(world, vehicle, frenet_map):
    global global_ob,global_glob_traj,global_obstacle
    ...
    path, all_paths = planner.frenet_optimal_planning(
                8,frenet_map.csp,
                s0, c_speed, c_d, c_d_d, c_d_dd, global_obstacle)
    ...
    if path is not None:
        global_glob_traj = []
        for i in range(5):
            global_glob_traj.append([path.x[i], path.y[i]])
    return path
...
def make_plan():
    global world, vehicle, frenet_map, global_path, global_pose, global_state, global_traj,global_glob_traj
    while True:
...
def decision(world, vehicle):
    global frenet_map, global_ob,global_obstacle
    ...
    for npc in vehicle_list:
        ...
        ob.append([x,y,theta])
        obstacle.append(obstacle_vehicle(npc))
    global_ob = ob
    global_obstacle = obstacle
    # print(npc_dict)
    # print()
    return npc_dict

def spaw_noise_vehicle(world, blueprint, vehicle, dis, auto=True):
    trans = vehicle.get_transform()
    yaw = trans.rotation.yaw / 180 * np.pi
    trans.location.x += dis * np.cos(yaw)
    trans.location.y += dis * np.sin(yaw)

    car = add_vehicle(world, blueprint, vehicle_type='vehicle.audi.a2',transform=trans)
    if auto:
        car.set_autopilot()
    return car
    
def main():
    global global_path, global_vel, global_view_img, global_collision, global_ob,global_obstacle
    global world, vehicle, frenet_map, global_pose, global_state, global_traj,global_glob_traj
    dynamic = True
    ...
    if dynamic:
        obstacle_car = spaw_noise_vehicle(world, blueprint, vehicle, 20,auto=False)
    else:
        obstacle_car = spaw_noise_vehicle(world, blueprint, vehicle, 5)
        ...
        if True:#global_path is not None:
            ...
            try:
                ...
                if global_path is not None:
                    ...
                    goal_pos_x.append(global_glob_traj[index][0])
                    goal_pos_y.append(global_glob_traj[index][1])
                    goal_speed.append(global_traj['v'][index])
                else:
                    goal_speed.append(0)
                    control = carla.VehicleControl(throttle=0.0, brake=1.0, steer=0.0)
                cmd_t.append(control.throttle)
                cmd_s.append(control.steer)
                vehicle.apply_control(control)

                if dynamic:
                    steer = 0.7 * math.sin((total_steps / 20) % 6)
                    obstacle_car.apply_control(carla.VehicleControl(throttle=0.5,  brake=0.0, steer=steer))
                # world.tick()s

                if len(global_t) > 20000:
                    """"""
                    ...
                    plt.subplot(2, 2, 3)
                    plt.xlabel(u't(s)', fontsize=10)  # 设置x轴，并设定字号大小
                    plt.ylabel(u'throttle', fontsize=10)  # 设置y轴，并设定字号大小
                    plt.plot(np.arange(len(cmd_t)), np.array(cmd_t), color='#6A5ACD')

                    plt.subplot(2, 2, 4)
                    plt.xlabel(u't(s)', fontsize=10)  # 设置x轴，并设定字号大小
                    plt.ylabel(u'steer', fontsize=10)  # 设置y轴，并设定字号大小
                    plt.plot(np.arange(len(cmd_s)), np.array(cmd_s), color='#6A5ACD')
