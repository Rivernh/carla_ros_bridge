...
class LatticePlanner:
    ...
    def check_collision(self, fp, ob):
        ...
        if test:
            if dynamic:
                for obstacle in ob:
                    car = [obstacle.pose[0] + obstacle.vel[0] * self.pl_cfg.dt * 5,
                           obstacle.pose[1] + obstacle.vel[1] * self.pl_cfg.dt * 5,
                           obstacle.pose[2]]
                    l = self.pl_cfg.L * 2
                    w = self.pl_cfg.W * 2
                    ...
                    ret_flag = []
                    cnt = 0
                    for (ix, iy, iyaw) in zip(fp.x, fp.y, fp.yaw):
                        left_bottom = [-l * math.cos(iyaw) + w * math.sin(iyaw) + ix,
                                       -l * math.sin(iyaw) - w * math.cos(iyaw) + iy]
                        right_bottom = [l * math.cos(iyaw) + w * math.sin(iyaw) + ix,
                                        l * math.sin(iyaw) - w * math.cos(iyaw) + iy]
                        right_up = [l * math.cos(iyaw) - w * math.sin(iyaw) + ix,
                                    l * math.sin(iyaw) + w * math.cos(iyaw) + iy]
                        left_up = [-l * math.cos(iyaw) - w * math.sin(iyaw) + ix,
                                   -l * math.sin(iyaw) + w * math.cos(iyaw) + iy]
                        box_traj = [left_bottom, right_bottom, right_up, left_up]
                        ret_flag.append(overlap(box_traj,box))
                        cnt += 1
                        if cnt > 10:
                            break
                        collision = any(ret_flag)
                        if collision:
                            return False
                    """
            else:
            ...
            for i in range(len(ob)):
                d = [((ix - ob[i].pose[0] - ob[i].vel[0] * self.pl_cfg.dt * 5) ** 2 + (iy - ob[i].pose[1]- ob[i].vel[1] * self.pl_cfg.dt * 5) ** 2)
                    for (ix, iy) in zip(fp.x, fp.y)]
